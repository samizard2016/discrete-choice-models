...
class DiscreteChoiceModel:
    ...
    def fit(self, df, target='Chosen', features=None):
        ...
        for col in ['FC', 'herd_size', 'cattle_type']:
        ...
        if features is None:
            features = ['Price', 'CP', 'FC']
            if len(mah_df['herd_size'].unique()) > 1:
                features.append('herd_size')
            if len(mah_df['cattle_type'].unique()) > 1:
                features.append('cattle_type')
        ...
        for market in self.markets:
            ...
            if self.use_pylogit:
                try:
                    ...
                    if 'herd_size' in features:
                        spec['herd_size'] = 'all_same'
                    if 'cattle_type' in features:
                        spec['cattle_type'] = 'all_same'
                    varnames = features
            ...
            if not self.use_pylogit:
                try:
                    # Statsmodels setup
                    categorical_cols = [col for col in ['FC', 'herd_size', 'cattle_type'] if col in features and col in market_df.columns]
                    ...
                    # Fit statsmodels with stronger regularization
                    model = sm.Logit(y, X).fit_regularized(method='l1', alpha=2.0, disp=0, maxiter=1000)
                    coef_df = pd.DataFrame({
