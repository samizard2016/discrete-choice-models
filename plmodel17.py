...
class DiscreteChoiceModel:
    ...
    def preprocess_data(self, df):
        ...
        # Log and handle duplicate brands in choice sets
        ...
        if not duplicates.empty:
            ...
            # Instead of dropping, take the first instance or aggregate (e.g., max Chosen)
            df = df.groupby(['ChoiceID', 'Brand']).agg({
                'Chosen': 'max',
                'Price': 'mean',
                'CP': 'mean',
                'Original_Price': 'mean',
            ...
            logging.info(f"Aggregated {len(duplicates)} duplicate choice sets by taking max Chosen and mean Price/CP")
        ...
        if df['Price'].nunique() <= 5:
            ...
            df['Price'] += np.random.normal(0, 0.01, len(df))
        ...
        if df['CP'].nunique() <= 5:
            ...
            df['CP'] += np.random.normal(0, 0.01, len(df))
    ...
    def balance_data(self, df, market, target):
        ...
        if market in ['Punjab', 'Maharashtra', 'Gujarat', 'South']:
            ...
            df = pd.concat([cargill_chosen, cargill_not_chosen, non_c-argill_chosen, non_cargill_not_chosen])
    ...
    def compute_std_errors(self, params, X, y, obs_id_col, alt_id_col, num_alts=3):
        ...
        try:
            ...
            std_errors = np.sqrt(np.maximum(np.diag(inv_hessian), 0))  # Ensure non-negative
            return std_errors
        except Exception as e:
    ...
    def add_model_summaries_to_workbook(self, workbook):
        ...
        for r, market in enumerate(self.markets, start=2):
            ...
            success = 'Success' if self.coef_dfs.get(market, pd.DataFrame()).notna().all().all() else 'Partial (No Std Errors)'
