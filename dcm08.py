...
class DiscreteChoiceAnalyzer:
    ...
    def validate_data(self):
        ...
        for _, row in choice_sets.iterrows():
            ...
            if 'Adv_Feat_TempMon' in set_encoded.columns:
                corr_tempmon = set_encoded[['Price', 'Adv_Feat_TempMon']].corr().iloc[0, 1]
                correlations_tempmon.append(corr_tempmon)
        ...
        if correlations_tempmon:
            mean_corr_tempmon = np.mean(correlations_tempmon)
            print(f"Mean Price-Adv_Feat_TempMon correlation in choice sets: {mean_corr_tempmon:.4f}")
            if np.abs(mean_corr_tempmon) > 0.5:
        ...
        for choice_set in self.choices['choice_set'].unique():
            ...
            for _, respondent in respondents.iterrows():
                ...
                for idx in profiles_presented:
                    ...
                    # Pool smaller groups into "Others"
    ...
    def evaluate_price_scenario(self, price_scenarios, plot=True):
        ...
        scenarios_df = (scenarios_df / 1000) - (self.price_mean / 1000)
