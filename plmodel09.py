...
class DiscreteChoiceModel:
    ...
    def fit(self, df, target='Chosen', features=None):
        ...
        for market in self.markets:
            ...
            if market_df.empty:
                ...
                self.price_sensitivity[market] = pd.DataFrame(columns=['Price_Change_Percent', 'Brand_Share', 'Value_Share', 'Actual_Brand_Share', 'Actual_Value_Share'])
    ...
    def simulate_price_sensitivity(self, market_df, market, brand='Cargill', features=None):
        if market not in self.models or isinstance(self.models[market], pd.DataFrame):
            ...
            return (pd.DataFrame(columns=['Price_Change_Percent', 'Brand_Share', 'Value_Share', 'Actual_Brand_Share', 'Actual_Value_Share']),
        ...
        # Get actual brand and value share for Cargill
        actual_brand_share = self.brand_share[(self.brand_share['Market'] == market) & (self.brand_share['Brand'] == brand)]['Actual_Brand_Share']
        actual_brand_share = actual_brand_share.iloc[0] if not actual_brand_share.empty else 0.0
        actual_value_share = self.value_share[(self.value_share['Market'] == market) & (self.value_share['Brand'] == brand)]['Actual_Value_Share']
        actual_value_share = actual_value_share.iloc[0] if not actual_value_share.empty else 0.0
        ...
        # Check for Raw_Price, else scale standardized prices to 10-100
        if 'Raw_Price' in market_df.columns and market_df['Raw_Price'].notna().any():
            print(f"Using Raw_Price for {market} value share calculations")
            price_col = 'Raw_Price'
            price_stats = market_df['Raw_Price'].describe()
        else:
            print(f"No Raw_Price for {market}, scaling Price to 10-100")
            price_col = 'Scaled_Price'
            min_price = market_df['Price'].min()
            max_price = market_df['Price'].max()
            price_range = max_price - min_price if max_price > min_price else 1.0
            market_df['Scaled_Price'] = 10 + 90 * (market_df['Price'] - min_price) / price_range
        ...
        for change in price_changes:
            ...
            for choice_id, group in choice_sets:
                ...
                if cargill_mask.any():
                    ...
                    # Update Scaled_Price or Raw_Price
                    if price_col == 'Scaled_Price':
                        min_price = group['Price'].min()
                        max_price = group['Price'].max()
                        price_range = max_price - min_price if max_price > min_price else 1.0
                        group['Scaled_Price'] = 10 + 90 * (group['Price'] - min_price) / price_range
                    elif price_col == 'Raw_Price' and 'Raw_Price' in group.columns:
                        base_raw_price = group.loc[cargill_mask, 'Raw_Price'].iloc[0]
                        if base_raw_price == 0:
                            print(f"Warning: Zero Raw_Price for Cargill in choice set {choice_id}, {market}. Skipping price adjustment.")
                            continue
                        group.loc[cargill_mask, 'Raw_Price'] = base_raw_price * (1 + change)
                ...
                for i, (_, row) in enumerate(group.iterrows()):
                    utilities.append({
                        ...
                        'Probability': probs[i],
                        'Price': row['Price'],
                        price_col: row[price_col] if price_col in row else row['Price']
                    })
                
                # Store utilities for all brands at 0% price change
                if change == 0:
                    for i, (_, row) in enumerate(group.iterrows()):
                        all_brands_utilities.append({
                            'ChoiceID': choice_id,
                            'Brand': row['Brand'],
                            'Probability': probs[i],
                            price_col: row[price_col] if price_col in row else row['Price']
            ...
            if utilities:
                ...
                print(f"{market} at {change*100:.2f}% price change: brand_share={brand_share:.4f}, brand_value={brand_value:.4f}, value_sum={value_sum:.4f}, value_share={value_share:.4f}, raw_weighted_prices={raw_weighted_prices:.4f}")
                
                results.append({
                    'Price_Change_Percent': change * 100,
                    'Brand_Share': brand_share,
                    'Value_Share': value_share,
                    'Actual_Brand_Share': actual_brand_share,
                    'Actual_Value_Share': actual_value_share
                })
        
        # Compute predicted brand and value shares for all brands at 0% price change
        if all_brands_utilities:
            all_brands_df = pd.DataFrame(all_brands_utilities)
            # Ensure probabilities sum to 1.0 per choice set
            choice_sum = all_brands_df.groupby(['ChoiceID', 'Brand'])['Probability'].sum().unstack().fillna(0)
            choice_sum = choice_sum.div(choice_sum.sum(axis=1), axis=0).fillna(0)
            predicted_brand_share = choice_sum.mean().reset_index(name='Predicted_Brand_Share')
            predicted_brand_share.columns = ['Brand', 'Predicted_Brand_Share']
            brand_share_sum = predicted_brand_share['Predicted_Brand_Share'].sum()
            print(f"Sum of Predicted_Brand_Share for {market}: {brand_share_sum:.4f}")
            if not np.isclose(brand_share_sum, 1.0, atol=0.01):
                print(f"Warning: Predicted_Brand_Share sum for {market} is {brand_share_sum:.4f}, adjusting to sum to 1.0")
            ...
            value_sum_all = (all_brands_df['Probability'] * all_brands_df[price_col]).sum()
            predicted_value_share = all_brands_df.groupby('Brand').apply(
            ...
            print(f"Sum of Predicted_Value_Share for {market}: {value_share_sum:.4f}")
            if not np.isclose(value_share_sum, 1.0, atol=0.01):
                print(f"Warning: Predicted_Value_Share sum for {market} is {value_share_sum:.4f}, adjusting to sum to 1.0")
                predicted_value_share['Predicted_Value_Share'] = predicted_value_share['Predicted_Value_Share'] / value_share_sum if value_share_sum > 0 else 0.0
            
            print(f"Predicted brand shares for {market}:\n{predicted_brand_share}")
            print(f"Predicted value shares for {market}:\n{predicted_value_share}")
        else:
            predicted_brand_share = pd.DataFrame(columns=['Brand', 'Predicted_Brand_Share'])
            predicted_value_share = pd.DataFrame(columns=['Brand', 'Predicted_Value_Share'])
        ...
        if not result_df.empty:
            ...
            if price_coef < 0 and not all(brand_share_diff <= 0):
                print(f"Warning: Brand_Share in {market} does not consistently decrease with price increase (price coef={price_coef:.4f})")
            elif price_coef > 0 and not all(brand_share_diff >= 0):
                print(f"Warning: Brand_Share in {market} does not consistently increase with price increase (price coef={price_coef:.4f})")
            value_share_diff = result_df['Value_Share'].diff().dropna()
            if price_coef < 0 and not all(value_share_diff <= 0):
                print(f"Warning: Value_Share in {market} does not consistently decrease with price increase (price coef={price_coef:.4f})")
        
        return (result_df if not result_df.empty else pd.DataFrame(columns=['Price_Change_Percent', 'Brand_Share', 'Value_Share', 'Actual_Brand_Share', 'Actual_Value_Share']),
                predicted_brand_share,
                predicted_value_share)
    ...
    def add_model_summaries_to_workbook(self, workbook):
        for market in self.markets:
            ...
            sensitivity_df = self.price_sensitivity.get(market, pd.DataFrame(columns=['Price_Change_Percent', 'Brand_Share', 'Value_Share', 'Actual_Brand_Share', 'Actual_Value_Share']))
            ...
            sensitivity_sheet['D1'] = 'Actual_Brand_Share'
            sensitivity_sheet['E1'] = 'Actual_Value_Share'
    ...
    def generate_price_sensitivity_charts(self):
        ...
        for market in self.markets:
            ...
            try:
                chart_data = sensitivity_df[['Price_Change_Percent', 'Brand_Share', 'Value_Share', 'Actual_Brand_Share', 'Actual_Value_Share']].to_dict(orient='records')
    ...
    <script type="text/babel">
        ...
        const App = () => (
            <LineChart width={{800}} height={{500}} data={{data}} margin={{ top: 20, right: 30, left: 20, bottom: 10 }}>
                ...
                <Line type="monotone" dataKey="Actual_Brand_Share" name="Actual Brand Share" stroke="#ff7300" strokeWidth={{2}} strokeDasharray="5 5" />
                <Line type="monotone" dataKey="Actual_Value_Share" name="Actual Value Share" stroke="#ff0000" strokeWidth={{2}} strokeDasharray="5 5" />
            </LineChart>
        );
...
if __name__ == "__main__":
    ...
    workbook.save('cargill_findings_v7.xlsx')