...
class DiscreteChoiceAnalyzer:
    ...
    def prepare_data(self):
        ...
        # Handle "Selected None" in chosen_profile
        choices_filtered['chosen_profile'] = choices_filtered['chosen_profile'].replace('Selected None', -1)
        choices_filtered['chosen_profile'] = pd.to_numeric(choices_filtered['chosen_profile'], errors='coerce')

        # Log the prevalence of "None" choices
        none_count = len(choices_filtered[choices_filtered['chosen_profile'] == -1])
        total_count = len(choices_filtered)
        self.logger.info(f"'Selected None' responses: {none_count} ({none_count/total_count*100:.2f}% of total choices)")
        ...
        choices_filtered = choices_filtered[choices_filtered['parsed_profiles'].apply(lambda x: all(p in valid_profiles for p in x) if x else False) | (choices_filtered['chosen_profile'] == -1)]
        self.logger.warning(f"Dropping {initial_rows - len(choices_filtered)} choices with invalid profiles or presentations")
        ...
        choices_filtered['profiles_presented'] = choices_filtered['parsed_profiles'].apply(
            lambda x: filter_profiles_presented(x, valid_profiles) if x else None
        )
                ...
                for idx in profiles_presented:
                    ...
                    row = {
                        **profile.to_dict(),
                        ...
                        'is_none': 0  # Indicator for profile (not "None")
                    }
                    choice_sets.append(row)
                
                # Add "None" option if chosen
                if respondent['chosen_profile'] == -1:
                    none_row = {
                        'Price': 0.0,
                        'Size_Perf_High': 0.0,
                        'Adv_Feat_ElecLife': 0.0,
                        'Adv_Feat_Health': 0.0,
                        'Adv_Feat_ModbusBasic': 0.0,
                        'Adv_Feat_Current': 0.0,
                        'Adv_Feat_ModbusEth': 0.0,
                        'Adv_Feat_Safety': 0.0,
                        'respondent_id': int(respondent['respondent_id']),
                        'choice_set': int(choice_set),
                        'group': str(group),
                        'chosen': 1,
                        'is_none': 1  # Indicator for "None" option
                    }
                    choice_sets.append(none_row)
        ...
        self.logger.info("Sample of choice_data:\n", self.choice_data[['respondent_id', 'choice_set', 'group', 'chosen', 'is_none']].head())
        ...
        numeric_cols = [col for col in self.choice_data.columns if col not in ['respondent_id', 'choice_set', 'group', 'chosen', 'is_none']]
    ...
    def fit_model(self, group=None, use_interactions=False):
        X_cols = [
            ...
            'Adv_Feat_Safety', 'Adv_Feat_Health', 'is_none'
        ]
    ...
    def evaluate_price_scenario_lp(self, price_scenarios, profile_labels=None, plot=True):
        ...
        profiles = self.profiles.copy()
        # print(f"Initial number of profiles: {len(profiles)}")
        profiles['Price'] = pd.to_numeric(profiles['Price'], errors='coerce')
        # print("NaN values in profiles:", profiles.isna().sum().to_dict())
        # Removed filtering: profiles = profiles[profiles['AF'] != 'Terminal Temperature threshold monitoring']
        ...
        profiles['AF'] = profiles['AF'].replace({
            ...
            # Optionally add: 'Terminal Temperature threshold monitoring': 'Some_Valid_AF_Category'
        })
        ...
        # print("Profiles_encoded index:", profiles_encoded.index.tolist())
        ...
        rename_dict = {
            ...
            'AF_Scalable connectivity at breaker level- Modbus Ethernet': 'Adv_Feat_ModbusEth',
            'AF_Operator Safety - Arc Flash reduction during maintenance': 'Adv_Feat_Safety'
            # Add if needed: 'AF_Terminal Temperature threshold monitoring': 'Adv_Feat_TerminalTemp'
        }
        profiles_encoded.rename(columns=rename_dict, inplace=True)
        ...
        # print("Scenarios_df index:", scenarios_df.index.tolist())
    ...
    def save_model(self, file_path):
        ...
        # print(f"Model and data saved to {file_path}_model.pkl and {file_path}_data.pkl")
    ...
    def restore_model(self, file_path):
        ...
        # print(f"Model and data restored from {file_path}_model.pkl and {file_path}_data.pkl")
        return self

if __name__ == "__main__":
    profiles = pd.read_excel('profiles.xlsx', index_col=0)
    choices = pd.read_excel("CBC_Data_Final_17July25.xlsx")
