...
class DiscreteChoiceModel:
    ...
    def simulate_price_sensitivity(self, market_df, market, brand='Cargill', features=None):
        ...
        return pd.DataFrame(results) if results else pd.DataFrame(columns=['Price_Change_Percent', 'Brand_Share', 'Value_Share', 'Actual_Brand_Share', 'Actual_Value_Share'])
    ...
    def add_model_summaries_to_workbook(self, workbook):
        for market in self.markets:
            ...
            if not importance_df.empty:
                start_row = len(coef_df) + 6  # Increased gap to avoid overlap
