...
class DiscreteChoiceModel:
    def __init__(self, markets=None, use_pylogit=True, output_dir="cattle_feed_output"):
        ...
        self.models = {market: None for market in self.markets}
        self.coef_dfs = {}
    ...
    def preprocess_data(self, df):
        ...
        # Log original brand shares
        for market in self.markets:
            market_df = df[df['Market'] == market]
            if not market_df.empty:
                original_shares = market_df[market_df['Chosen'] == 1].groupby('Brand').size() / market_df[market_df['Chosen'] == 1].shape[0]
                logging.info(f"Original brand shares in {market}:\n{original_shares.reset_index(name='Original_Brand_Share')}")
        ...
        # Group low-frequency brands, ensuring Baramati is retained in Maharashtra
        for market in self.markets:
            market_df = df[df['Market'] == market]
            brand_counts = market_df['Brand'].value_counts(normalize=True)
            major_brands = brand_counts[brand_counts >= 0.05].index.tolist()
            if market == 'Maharashtra':
                if 'Baramati' not in major_brands and 'Baramati' in brand_counts:
                    major_brands.append('Baramati')
                    logging.info(f"Added Baramati to major brands in Maharashtra (share: {brand_counts.get('Baramati', 0):.4f})")
            df.loc[df['Market'] == market, 'Brand'] = df.loc[df['Market'] == market, 'Brand'].where(df['Brand'].isin(major_brands), 'Other')
            logging.info(f"Major brands in {market} (share >= 5% or Baramati): {major_brands}")
            logging.info(f"Brands grouped into 'Other' in {market}: {set(market_df['Brand'].unique()) - set(major_brands)}")
    ...
    def balance_data(self, df, market, target):
        """Balance Cargill vs. non-Cargill choices for specified markets, except Maharashtra."""
        if market in ['Punjab', 'Gujarat', 'South']:
            ...
            logging.info(f"Skipping balancing for {market} to preserve natural brand shares (e.g., Baramati)")
        return df
    ...
    def fit(self, df, target='Chosen', features=None):
        ...
        for market in self.markets:
            ...
            logging.info(f"Initial {market} data: {len(market_df)} rows")
            market_df = self.balance_data(market_df, market, target)
            logging.info(f"Post-balancing {market} data: {len(market_df)} rows")
            if len(market_df) < 50:
                logging.warning(f"Insufficient data for {market} after balancing: {len(market_df)} rows")
        ...
        for market in self.markets:
            ...
            if len(valid_obs) < 10:
                ...
                self.coef_dfs[market] = pd.DataFrame()
                continue
            logging.info(f"{market} choice set size distribution:\n{choice_counts.value_counts().sort_index()}")
    ...
    def add_model_summaries_to_workbook(self, workbook):
        ...
        for r, market in enumerate(self.markets, start=2):
            ...
            notes = 'Skipped balancing to preserve natural brand shares (e.g., Baramati)' if market == 'Maharashtra' else ''
            summary_sheet[f'A{r}'] = market
            summary_sheet[f'B{r}'] = model_type
            summary_sheet[f'C{r}'] = success
            summary_sheet[f'D{r}'] = notes
