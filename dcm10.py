...
class DiscreteChoiceAnalyzer:
        ...
        for choice_set in choices_filtered['choice_set'].unique():
            ...
            for _, respondent in respondents.iterrows():
                ...
                for idx in profiles_presented:
                    ...
                    # group = group_row['group'].iloc[0] # to be removed
                    row = {
                        **profile.to_dict(),
    ...
    def plot_feature_importance(self, custom_labels=None, max_label_length=30, wrap_width=20):
        ...
        # Use custom_labels if provided, otherwise fall back to default_labels
        label_map = custom_labels if custom_labels is not None else default_labels
        
        # Create DataFrame with feature importance
        importance_df = pd.DataFrame(list(self.feature_importance.items()), columns=['Feature', 'Importance'])
        
        # Map feature names to custom labels and wrap/abbreviate long names
        ...
        # Add left margin to prevent truncation
