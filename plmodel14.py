...
class DiscreteChoiceModel:
    ...
    def fit(self, df, target='Chosen', features=None):
        ...
        if len(df['Price'].unique()) <= 3:
            print("Warning: Price has only 3 unique values. Adding synthetic variation.")
            df['Price'] = df['Price'] + np.random.normal(0, 0.01, len(df))
        if len(df['CP'].unique()) <= 3:
            print("Warning: CP has only 3 unique values. Adding synthetic variation.")
            df['CP'] = df['CP'] + np.random.normal(0, 0.01, len(df))
        ...
        if features is None:
            features = ['Price', 'CP', 'FC', 'AH', 'VAS', 'Credit']
        categorical_cols = ['FC', 'AH', 'VAS', 'Credit']
        for col in categorical_cols:
            if col in df.columns:
                ...
                dummy_cols = [c for c in dummies.columns if c != f"{col}_None"]
                if col in features:
                    features.remove(col)
        ...
        for market in self.markets:
            ...
            try:
                ...
                init_vals = np.zeros(len(features))
                bounds = [(-100, 100)] * len(features)
                ...
                for _, row in group.iterrows():
                    ...
                    for var in features:
                        if var in coefs.index and var in row.index:
                            value = float(row[var]) if isinstance(row[var], (int, float, np.number)) else 0.0
                            utility += coefs[var] * value
...
if __name__ == "__main__":
    ...
    workbook.save('cargill_findings_v14.xlsx')