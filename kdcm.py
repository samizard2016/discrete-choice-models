...
class DCMApp(QMainWindow):
    def __init__(self):
        ...
        # Define DCM and BDCM descriptions first to avoid overwrites
        self.dcm_text = (
        ...
        # # Configure logging
        # log_file = os.path.join(os.getcwd(), 'kdcm_simulator.log')
        # logging.basicConfig(
        ...
        # self.logger.debug("Debugging log test")
        for handler in self.logger.handlers:
            if isinstance(handler, logging.FileHandler):
                handler.flush()
        # Verify handlers
        # print(f"Root logger handlers: {logging.getLogger().handlers}")
        # print(f"kdcm_logger handlers: {logging.getLogger('kdcm_logger').handlers}")
         # Create named logger
        self.logger = logging.getLogger("kdcm_logger")
        self.logger.setLevel(logging.DEBUG)
        ...
        # Moved after all initializations to avoid premature calls       
        if self.check_reg():
    ...
    #             if unnamed_cols:
    #                 self.logger.warning(f"Dropping Unnamed columns from {df}: {unnamed_cols}")
    #                 df.drop(columns=unnamed_cols, inplace=True)
    #         try:
    #             self.logger.info("About to restore the trained model...")
    ...
    #             self.logger.info("About to evaluate new price scenarios...")
    #             d_model_for = {"All":"All","Panel Builders":"Panel builders","Non-Panel Builders":"Others"}
    #             if self.model_for=="All":
    #                 analyzer = analyzer.restore_model("schneider_choice_model")
    #                 self.logger.info("model has been restored successfully")
    #                 # print("model restored")
    #                 shares = analyzer.evaluate_price_scenario_lp(d_dict, plot=False)
    #             elif self.model_for=="Panel Builders":
    ...
    #                 analyzer = DiscreteChoiceAnalyzer(profiles, choices, groups)             
    #                 self.logger.info(f"Choice data prepared for {self.model_for}, rows: {len(choice_data)}")      
    #                 utilities = analyzer.fit_model(group=d_model_for[self.model_for], use_interactions=False)
    #                 importance = analyzer.calculate_feature_importance()
    #                 shares = analyzer.evaluate_price_scenario_lp(d_dict, plot=False,file=f"profile_shares_{self.model_for}")
    #                 self.logger.info(f"Shares predicted for {self.model_for}")
    #             elif self.model_for=="Non-Panel Builders":
    #                 choice_data = analyzer.prepare_data()
    #                 self.logger.info(f"Choice data prepared for {self.model_for}, rows: {len(choice_data)}")
    #                 utilities = analyzer.fit_model(group=d_model_for[self.model_for], use_interactions=False)
    #                 shares = analyzer.evaluate_price_scenario_lp(d_dict, plot=False,file=f"profile_shares_{self.model_for.replace('-','_')}")
    #             self.logger.info(f"shares predicted for {self.model_for}")
    #             shares['Profile'] = df['Profile']
    #             shares = shares[list(df.columns)]
    #             out_file = f"simulated shares for {self.model_for} {os.path.basename(data_file)}"
    #             shares.to_excel(out_file, index=False)
    #             shares.to_excel(out_file, sheet_name='Profile Share', index=False, engine='openpyxl')
    #             self.create_profile_share_chart(out_file)
    #             msg = f"simulated shares have been saved to {out_file}"
    #             self.logger.info(msg)
    #             self.update_status(msg)
    #         except Exception as e:
    #             self.logger.error(f"Error occurred in simulation: {str(e)}")
    #             self.update_status(str(e))
    #     except Exception as err:
    #         self.logger.error("Error found in reading the data files: {err}")


    def get_base_dir(self):
        if getattr(sys, 'frozen', False):
            # Running as a frozen executable (cx_Freeze)
            base_dir = os.path.dirname(sys.executable)
            # Optionally, adjust if resources are in a subdirectory (e.g., 'data')
    ...
    def run_simulate(self):
        ...
        try:
            ...
            try:
                ...
                shares.to_excel(out_file, index=False, sheet_name='Profile Share', engine='openpyxl')
                self.create_profile_share_chart(out_file)
                msg = f"Simulated shares have been saved to {out_file}"
                self.logger.info(msg)
                self.update_status(msg)
    ...
    def set_model_text(self):
        ...
        print(f"self.dcm_text type in set_model_text: {type(self.dcm_text)}")
    ...
    def update_dashboard(self):
        ...
        if self.radio_DCM.isChecked():
            ...
            d_int = {"utilities_with_uncertainty_bdcm.png":
    ...
    def create_profile_share_chart(self, input_file="simulated_share.xlsx", data_sheet="Profile Share"):
        try:
            self.logger.info("Creating new profile share plot...")
            # Load the workbook
            ...
            if "plot" in workbook.sheetnames:
                workbook.remove(workbook["Plot"])  # Remove existing "plot" sheet to avoid conflicts
            ...
            # Add each price column as a series with reduced line thickness
            ...
            self.logger.info(f"Profile share plot saved to {input_file}")
            # Flush logs
            for handler in self.logger.handlers:
                if isinstance(handler, logging.FileHandler):
                    handler.flush()
