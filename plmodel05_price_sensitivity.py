    ...
    def simulate_price_sensitivity(self, market_df, market, brand='Cargill', features=None):
        ...
        for change in price_changes:
            ...
            for choice_id, group in choice_sets:
                ...
                group_utilities = np.zeros(len(group))
                    ...
                    for var in coefs.index:
                        if var == 'Price':
                            ...
                            if row[var_name] == var_value:
                                utility += coefs[var]
                    group_utilities[idx] = utility
                
                # Compute choice probabilities (softmax)
                exp_utilities = np.exp(group_utilities)
                probs = exp_utilities / exp_utilities.sum() if exp_utilities.sum() > 0 else np.ones(len(group)) / len(group)
                
                # Assign probabilities to alternatives
                for i, (idx, row) in enumerate(group.iterrows()):
                    utilities.append({'ChoiceID': choice_id, 'Brand': row['Brand'], 'Probability': probs[i], 'Price': row['Price']})
