...
class DiscreteChoiceModel:
    ...
    def fit(self, df, target='Chosen', features=None):
        ...
        if features is None:
            features = ['Price', 'CP']
            for col in ['FC', 'AH', 'VAS', 'Credit']:
                if col in categorical_cols:
                    dummy_cols = [c for c in df.columns if c.startswith(f"{col}_") and c != f"{col}_None"]
                    if dummy_cols:
        ...
        for market in self.markets:
            ...
            try:
                ...
                # Fallback to binary logit using statsmodels
                try:
...
if __name__ == "__main__":
    ...
    workbook.save('cargill_findings_code_v13.xlsx')