...
class DiscreteChoiceAnalyzer:
    ...
    def validate_data(self):
        ...
        self.logger.warning(f"Invalid chosen profiles not in profiles_presented: {len(invalid)}")
        if len(invalid) > 0:
            self.logger.warning("Some chosen profiles are not in profiles_presented, indicating data errors.")
            self.logger.warning("Invalid choices sample:\n", invalid[['chosen_profile', 'profiles_presented']].head())
        ...
        if correlations_modbus:
            self.logger.warning(f"Mean Price-Adv_Feat_ModbusBasic correlation: {np.mean(correlations_modbus):.4f}")
        else:
        ...
        self.logger.info("Respondent choice counts:\n", respondent_choices.value_counts())
        ...
        self.logger.info("Choice sets per respondent:\n", choice_set_counts.value_counts())
    ...
    def prepare_data(self):
        ...
        self.logger.info("Uncentered Price range:\n", profiles['Price'].describe())
        ...
        self.logger.info("Unique SP values:", profiles['SP'].unique())
        self.logger.info("Unique AF values:", sorted(profiles['AF'].unique()))
        self.logger.info("Dropped AF level (first alphabetically):", sorted(profiles['AF'].unique())[0])
        ...
        self.logger.info("profiles_encoded columns before renaming:", profiles_encoded.columns.tolist())
        ...
        self.logger.info("profiles_encoded columns after renaming:", profiles_encoded.columns.tolist())
        self.logger.info("profiles_encoded shape:", profiles_encoded.shape)
        ...
        self.logger.warning(f"Dropping {initial_rows - len(choices_filtered)} choices with invalid profiles (chosen or presented): {set(self.choices['chosen_profile'].unique()) - set(valid_profiles)}")
        ...
        self.logger.warning(f"Dropping {len(invalid_sets)} choice sets with fewer than 2 valid profiles")
        ...
        self.logger.info("Price range (centered, thousands):\n", self.choice_data['Price'].describe())
        
        self.logger.info("Group value counts:\n", self.choice_data['group'].value_counts())
        self.logger.info("Sample of choice_data:\n", self.choice_data[['respondent_id', 'choice_set', 'group', 'chosen']].head())
        self.logger.info("Final choice_data columns:", self.choice_data.columns.tolist())
        self.logger.info("Final choice_data dtypes:\n", self.choice_data.dtypes)
    ...
    def fit_model(self, group=None, use_interactions=False):
        ...
        print(f"Fitting model for {'all groups' if group is None else group}, interactions={use_interactions}")
        print("X_cols:", X_cols)
    ...
    def plot_utilities(self, custom_labels=None):
        ...
        default_labels = {
            ...
            'Price_Panel': 'Price x Panel Builder'
        }
    ...
    def plot_feature_importance(self, custom_labels=None, max_label_length=30, wrap_width=20):
        ...
        importance_df['Feature'] = importance_df['Feature'].map(
            lambda x: '\n'.join(textwrap.wrap(label_map.get(x, x), wrap_width)) 
            if len(label_map.get(x, x)) > max_label_length 
            else label_map.get(x, x)
        )
        ...
        plt.savefig('feature_importance_dcm.png', dpi=400, bbox_inches='tight')
        plt.close()
    ...
    def evaluate_price_scenario_lp(self, price_scenarios, profile_labels=None, plot=True, file=None):
        if self.model is None:
            raise ValueError("Model is not fitted yet. Call fit_model() first.")
        ...
        profiles_encoded['Price'] = -profiles['Price'].values / 1000
        ...
        for scenario in scenarios_df.columns:
            ...
            for profile_idx in scenarios_df.index:
                if profile_idx in temp_profiles.index:
                    ...
                    self.logger.warning(f"Profile {profile_idx} not found in temp_profiles for scenario {scenario}")
        ...
        self.logger.info(f"Shares shape: {shares.shape}")
        self.logger.info("Shares index:", shares.index.tolist())
    ...
    def estimate_value_share(self, price_scenarios, profile_labels=None, plot=True, file=None):
        """
        Estimate the value share (revenue share) for each profile under different price scenarios.
        
        Parameters:
        -----------
        price_scenarios : dict or pd.DataFrame
            Dictionary or DataFrame where keys/columns are scenario names and values are prices for each profile.
        profile_labels : dict or list, optional
            Labels for profiles, either as a dictionary mapping indices to labels or a list of labels.
        plot : bool, optional
            If True, generate a line plot of value shares across scenarios.
        file : str, optional
            File path to save the plot. If None, defaults to 'value_shares_line.png'.
        
        Returns:
        --------
        pd.DataFrame
            DataFrame with value shares (%) for each profile under each scenario.
        """
        if self.model is None:
            raise ValueError("Model is not fitted yet. Call fit_model() first.")
        
        # Get choice shares from evaluate_price_scenario_lp
        shares = self.evaluate_price_scenario_lp(price_scenarios, profile_labels=profile_labels, plot=False) / 100
        
        # Convert price_scenarios to DataFrame if it's a dict
        if isinstance(price_scenarios, dict):
            scenarios_df = pd.DataFrame(price_scenarios, dtype=float)
        else:
            scenarios_df = price_scenarios.astype(float)
        
        # Calculate value shares (price * share)
        value_shares = pd.DataFrame(index=shares.index, columns=shares.columns)
        for scenario in scenarios_df.columns:
            for profile_idx in scenarios_df.index:
                if profile_idx in shares.index:
                    price = scenarios_df.loc[profile_idx, scenario]
                    share = shares.loc[profile_idx, scenario]
                    value_shares.loc[profile_idx, scenario] = price * share
                else:
                    self.logger.warning(f"Profile {profile_idx} not found in shares for scenario {scenario}")
        
        # Normalize value shares to sum to 100% per scenario
        value_shares = value_shares.div(value_shares.sum(axis=0), axis=1) * 100
        
        self.logger.info(f"Value shares shape: {value_shares.shape}")
        self.logger.info("Value shares index:", value_shares.index.tolist())
        
        # Handle profile labels
        if profile_labels is None:
            profile_labels = [f'Profile {i}' for i in shares.index]
        elif isinstance(profile_labels, dict):
            profile_labels = [profile_labels.get(i, f'Profile {i}') for i in shares.index]
        elif isinstance(profile_labels, list):
            if len(profile_labels) != len(shares.index):
                raise ValueError("profile_labels list length must match number of profiles")
        
        # Plotting
        if plot:
            plot_data = value_shares.reset_index().melt(id_vars='index', var_name='Scenario', value_name='Value_Share')
            plot_data['Profile'] = plot_data['index'].map({i: label for i, label in enumerate(profile_labels)})
            
            plt.figure(figsize=(12, 6))
            sns.lineplot(data=plot_data, x='Profile', y='Value_Share', hue='Scenario', style='Scenario', markers=True, dashes=False)
            plt.title('Value Shares Across Price Scenarios')
            plt.xlabel('Profile')
            ...
            if file is not None:
                ...
                plt.savefig('value_shares_line.png')
            plt.close()
    ...
    def save_model(self, file_path):
        """
        Save the trained logit model and associated data to disk for profile share prediction.
        
        Parameters:
        -----------
        file_path : str
            Path where the model and data will be saved (without extension)
        """
    ...
    def restore_model(self, file_path):
        """
        Restore a previously saved logit model and associated data for profile share prediction.
        
        Parameters:
        -----------
        file_path : str
            Path to the saved model files (without extension)
        
        Returns:
        --------
        self : DiscreteChoiceAnalyzer
            The instance with restored model and data
        """
        try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        choice_data.to_excel("choice_final_data.xlsx", index=False)
        ...
        df_scenarios.to_excel("Scenarios.xlsx", index=False)
        
        shares = analyzer.evaluate_price_scenario_lp(price_scenarios, plot=True)
        print("Profile Shares (%):\n", shares.round(2))
        
        value_shares = analyzer.estimate_value_share(price_scenarios, plot=True)
        print("Value Shares (%):\n", value_shares.round(2))
        ...
        for group in choice_data['group'].unique():
            ...
            utilities.to_excel(f"utilities_{group}.xlsx")
            shares = analyzer.evaluate_price_scenario_lp(price_scenarios, plot=True, file=f"profile_shares_{group}")
            print(f"Profile Shares for {group} (%):\n", shares.round(2))
            value_shares = analyzer.estimate_value_share(price_scenarios, plot=True, file=f"value_shares_{group}")
            print(f"Value Shares for {group} (%):\n", value_shares.round(2))
