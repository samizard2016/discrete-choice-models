...
class DiscreteChoiceModel:
    ...
    def simulate_price_sensitivity(self, market_df, market, brand='Cargill', features=None):
        ...
        actual_brand_share = self.brand_share[(self.brand_share['Market'] == market) & (self.brand_share['Brand'] == brand)]['Brand_Share']
        actual_brand_share = actual_brand_share.iloc[0] if not actual_brand_share.empty else 0.0
        actual_value_share = self.value_share[(self.value_share['Market'] == market) & (self.value_share['Brand'] == brand)]['Value_Share']
        ...
        price_adjustment = abs(min_price) + 0.1 if min_price < 0 else 0.0
        ...
        print(f"Price stats for {market}: min={price_stats['min']:.4f}, max={price_stats['max']:.4f}, mean={price_stats['mean']:.4f}, std={price_stats['std']:.4f}")
        print(f"Price adjustment for {market}: {price_adjustment:.4f} (added to ensure positive prices)")
        ...
        for change in price_changes:
            ...
            if utilities:
                ...
                # Normalize prices for value share to ensure positive values
                normalized_prices = util_df['Price'] + price_adjustment
                value_sum = (util_df['Probability'] * normalized_prices).sum()
                brand_value = (util_df[util_df['Brand'] == brand]['Probability'] * (util_df[util_df['Brand'] == brand]['Price'] + price_adjustment)).sum() if brand in util_df['Brand'].values else 0.0
    ...
    def add_model_summaries_to_workbook(self, workbook):
        for market in self.markets:
            ...
            for r, (index, row) in enumerate(sensitivity_df.iterrows(), start=2):
                ...
                sensitivity_sheet[f'D{r}'] = row['Actual_Brand_Share']
                sensitivity_sheet[f'E{r}'] = row['Actual_Value_Share']
