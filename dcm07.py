...
class DiscreteChoiceAnalyzer:
    ...
    def validate_data(self):
        """Check for data anomalies and choice set correlations."""
        choice_data = self.choice_data.copy()
        choice_data['Price'] = choice_data['Price'] * 1000  # Convert back to dollars
        ...
        for _, row in choice_sets.iterrows():
            ...
            if 'Adv_Feat_TempMon' in set_encoded.columns:
                corr = set_encoded[['Price', 'Adv_Feat_TempMon']].corr().iloc[0, 1]
                correlations.append(corr)
        if correlations:
            print(f"Mean Price-Adv_Feat_TempMon correlation in choice sets: {np.mean(correlations):.4f}")
            if np.abs(np.mean(correlations)) > 0.5:
                print("Warning: High Price-Adv_Feat_TempMon correlation in choice sets.")
        return price_choice
    ...
    def prepare_data(self):
        ...
        self.choice_data['Price_Size_Interaction'] = self.choice_data['Price'] * self.choice_data['Size_Perf_High']
    ...
    def fit_model(self, group=None):
        X_cols = [
            ...
            'Adv_Feat_Safety', 'Adv_Feat_ModbusBasic', 'Adv_Feat_ModbusEth',
            'Adv_Feat_Health', 'Price_Size_Interaction'
        ]
        ...
        try:
            self.model = sm.Logit(y, X).fit(disp=0, maxiter=1000, method='bfgs')
            if not self.model.mle_retvals['converged']:
                print("bfgs failed to converge, trying nm...")
                self.model = sm.Logit(y, X).fit(disp=0, maxiter=1000, method='nm')
        except Exception as e:
    ...
    def calculate_feature_importance(self):
        ...
        coef = self.model.params[1:].values
    ...
    def plot_utilities(self, custom_labels=None):
        ...
        default_labels = {
            ...
            'Price_Size_Interaction': 'Price × Size Interaction'
        }
    ...
    def evaluate_price_scenario(self, price_scenarios, plot=True):
        ...
        profiles_encoded['Price_Size_Interaction'] = profiles_encoded['Price'] * profiles_encoded['Size_Perf_High']
        ...
        for scenario in scenarios_df.columns:
            ...
            for profile_idx in scenarios_df.index:
                if profile_idx in temp_profiles.index:
                    ...
                    temp_profiles.loc[profile_idx, 'Price_Size_Interaction'] = temp_profiles.loc[profile_idx, 'Price'] * temp_profiles.loc[profile_idx, 'Size_Perf_High']
...
if __name__ == "__main__":
    ...
    try:
        ...
        for group in choice_data['group'].unique():
            analyzer.fit_model(group=group)
            print(f"\nUtilities for {group}:\n", analyzer.utilities)
            shares = analyzer.evaluate_price_scenario_lp(price_scenarios, plot=True)
