...
class DiscreteChoiceAnalyzer:
    ...
    def prepare_data(self):
        ...
        self.price_mean = profiles['Price'].mean()
        self.price_std = profiles['Price'].std()
        profiles['Price'] = (profiles['Price'] - self.price_mean) / self.price_std
        ...
        profiles_encoded = pd.get_dummies(
            ...
            columns=['SP', 'AF'],
            drop_first=True,
            dtype=float  # Changed to float for consistency
        )
    ...
    def fit_model(self):
        ...
        try:
            self.model = sm.Logit(y, X).fit(disp=0, maxiter=500, method='bfgs')
    ...
    def calculate_feature_importance(self):
        ...
        # Use LinearExplainer with model coefficients
        coef = self.model.params[1:].values  # Exclude constant
    ...
    def plot_price_elasticity(self):
        ...
        temp_data['original_price'] = temp_data['Price'] * self.price_std + self.price_mean
    ...
    def evaluate_price_scenario(self, price_scenarios, plot=True):
        ...
        profiles = self.profiles.copy()
        profiles['Price'] = pd.to_numeric(profiles['Price'], errors='coerce', downcast='float')
        ...
        scenarios_df = (scenarios_df - self.price_mean) / self.price_std
